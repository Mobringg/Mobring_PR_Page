...
export async function getDynamicProjects({ onlyPublished = false } = {}) {
  ...
  // manifest.json은 계속 같은 경로를 덮어쓰는 파일이라, Blob의 CDN이 이전 내용을
  // 한동안 캐시해서 내려줄 수 있습니다. 매번 다른 쿼리 파라미터를 붙여 반드시
  // 새로 요청하도록 강제합니다 (URL이 달라지면 CDN 캐시를 새로 조회하게 됨).
  const bustCacheUrl = `${manifestBlob.url}?t=${Date.now()}`;
  const res = await fetch(bustCacheUrl, { cache: "no-store" });
  if (!res.ok) return [];
  const items = await res.json();

  return onlyPublished ? items.filter((item) => item.published) : items;
}
...
async function saveDynamicProjects(items) {
  await put(MANIFEST_PATH, JSON.stringify(items, null, 2), {
    ...
    cacheControlMaxAge: 0, // manifest.json은 자주 바뀌므로 CDN에 오래 캐시되지 않도록 지정
  });
}
 ...
 * 처음 추가할 때는 항상 비공개(published: false) 상태로 저장되어,
 * 실제 방문자 페이지(/portfolio)에는 노출되지 않습니다.
 * 관리자 페이지에서 "공개로 전환" 버튼을 눌러야 비로소 노출됩니다.
 */
export async function addDynamicProject({
  ...
  const newItem = {
    ...
    published: false, // 기본값은 비공개(초안) — 명시적으로 공개 전환해야 노출됨
  };
...
export async function setPublished(id, published) {
  ...
  await saveDynamicProjects(updated);
